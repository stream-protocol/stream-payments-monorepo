...
import (
	...
	"github.com/stream-payments/streampayment/internal/config"
	"github.com/stream-payments/streampayment/internal/event/paymentevents"
	"github.com/stream-payments/streampayment/internal/event/userevents"
	"github.com/stream-payments/streampayment/internal/locator"
	"github.com/stream-payments/streampayment/internal/log"
	"github.com/stream-payments/streampayment/internal/scheduler"
	httpServer "github.com/stream-payments/streampayment/internal/server/http"
	"github.com/stream-payments/streampayment/internal/server/http/internalapi"
	"github.com/stream-payments/streampayment/internal/server/http/merchantapi"
	merchantauth "github.com/stream-payments/streampayment/internal/server/http/merchantapi/auth"
	"github.com/stream-payments/streampayment/internal/server/http/paymentapi"
	"github.com/stream-payments/streampayment/internal/server/http/webhook"
	"github.com/stream-payments/streampayment/internal/service/user"
	"github.com/stream-payments/streampayment/pkg/graceful"
	uidashboard "github.com/stream-payments/streampayment/ui-dashboard"
	uipayment "github.com/stream-payments/streampayment/ui-payment"
	"github.com/stream-payments/streampayment/web"
...
func New(ctx context.Context, cfg *config.Config) *App {
	...
	logger := log.New(cfg.Logger, "streampayment", cfg.GitVersion, cfg.Env, hostname)
...
func (app *App) RunServer() {
	...
	dashboardAuthHandler := merchantauth.NewHandler(
		...
		app.config.StreamPayment.Auth.EnabledProviders(),
		app.Logger(),
	)
	...
	withInternalAPI := app.config.StreamPayment.Server.EnableInternalAPI

	srv := httpServer.New(
		app.config.StreamPayment.Server,
		app.config.Debug,
		...
		httpServer.WithDashboardAPI(
			...
			app.services.TokenManagerService(),
			app.services.UserService(),
			app.config.StreamPayment.Auth.Email.Enabled,
			app.config.StreamPayment.Auth.Google.Enabled,
		),
		httpServer.WithPaymentAPI(paymentAPIHandler, app.config.StreamPayment.Server),
	...
	app.OnBeforeRun(func(ctx context.Context, app *App) error {
		cfg := app.config.StreamPayment.Auth.Email
		if !cfg.Enabled {
			return nil
		}
